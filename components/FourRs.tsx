...
function RCard({ type, offering, icon, title, tagline, description, buttonText }: RCardProps) {
  ...
  return (
    <div
      className={`bg-white rounded-3xl p-8 border border-border shadow-medium transition-[transform,box-shadow,border-color] duration-300 text-center flex flex-col hover:-translate-y-1.5 ${styles.hoverBorder} ${styles.hoverShadow}`}
    >
      <a
        href="#"
        className={`inline-flex items-center justify-center gap-2 py-2.5 px-4 rounded-full text-sm font-semibold text-white transition-[transform,filter] duration-300 hover:-translate-y-0.5 hover:brightness-110 mx-auto mb-4 ${styles.buttonGradient}`}
      >
        {buttonText}
      </a>
